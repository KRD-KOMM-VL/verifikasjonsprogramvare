...
public class EncryptedVoteCompressionUnitTest {
    ...
    private static final BigInteger MESSAGE_COMPONENT_OF_COMPRESSED_ENC_OPT_IDS = new BigInteger(
            "");
    private ElGamalEncryptionTuple originalEncOptIds;
    private ElGamalEncryptionTuple compressedEncOptIds;
    private CleansedVote cleansedVote;

    /**
     * Calculates the multiplied encrypted voting option IDs to be used by the
     * tests.
     */
    @BeforeTest
    public void createMultipliedEncOptIds() {
        ...
        compressedEncOptIds = encryptedVote.getCompressedEncVoteOptIds(AREA_PRIME,
                ENVIRONMENT_PRIME, MODULUS);
    }

    /**
     * Creates the cleansed vote.
     */
    @BeforeTest
    ...
    /**
     * Verifies that the public key component of the compressed encrypted vote option IDs is
     * the same as the public key component in the original encrypted vote option IDs.
     */
    @Test
    public void mustCopyPublicKeyComponentFromOriginalToCompressedEncOptIds() {
        assertEquals(compressedEncOptIds.getPublicKeyComponent(),
            originalEncOptIds.getPublicKeyComponent());
    }

    /**
     * Verifies that the message component of the compressed encrypted vote option IDs is
     * calculated correctly.
     *
     * TODO
     */
    @Test(enabled = false)
    public void mustCalculateMessageComponentOfCompressedEncOptIdsCorrectly() {
        assertEquals(compressedEncOptIds.getMessageComponents(),
            MESSAGE_COMPONENT_OF_COMPRESSED_ENC_OPT_IDS);
    }

    /**
     * Verifies that the public key component of the compressed encrypted vote option IDs matches the public key component
     * of the cleansed vote.
     */
    @Test
    public void publicKeyComponentOfCompressedEncOptIdsCorrectlyMustMatchPublicKeyComponentOfCleansedVote() {
        assertEquals(compressedEncOptIds.getPublicKeyComponent(),
            cleansedVote.getEncryptedVoteOptIds().getPublicKeyComponent());
    }

    /**
     * Verifies that the message component of the compressed encrypted vote option IDs matches the message component of the cleansed vote.
     *
     * TODO
     */
    @Test(enabled = false)
    public void messageComponentOfCompressedEncOptIdsCorrectlyMustMatchMessageComponentOfCleansedVote() {
        assertEquals(compressedEncOptIds.getMessageComponents(),
