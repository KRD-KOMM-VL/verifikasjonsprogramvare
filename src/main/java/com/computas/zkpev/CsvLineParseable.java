...
public abstract class CsvLineParseable {
    ...
    protected String getAttributeAsString(String[] attributes, Enum index) {
        return getAttribute(attributes, index).replace("#r#", "\r")
                   .replace("#n#", "\n");
    }
    ...
    protected byte[] getAttributeAsMultilinedByteArray(String[] attributes,
        Enum index) {
        return Base64.decodeBase64(getAttribute(attributes, index)
                                       .replace("#n#", "\n"));
    }
    ...
    protected byte[][] getAttributeAsByteArrayArray(String[] attributes,
        Enum index) {
        return getAttributeAsByteArrayArray(attributes, index, false);
    }

    protected byte[][] getAttributeAsMultilinedByteArrayArray(
        String[] attributes, Enum index) {
        return getAttributeAsByteArrayArray(attributes, index, true);
    }

    protected byte[][] getAttributeAsByteArrayArray(String[] attributes,
        Enum index, boolean multiline) {
        String byteArrayArrayString = multiline
            ? getAttributeAsString(attributes, index)
            : getAttribute(attributes, index);
